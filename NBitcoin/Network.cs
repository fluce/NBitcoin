...
namespace NBitcoin
{
	...
	public class Network
	{
		...
		public static Network RegTest
		{
			get
			{
				if(_RegTest == null)
				{
					var instance = new Network(WellKnownNetworkInitDataFactory.InitReg(new NetworkInitData()));
					_RegTest = instance;
				}
				return _RegTest;
			}
        ...
        private Network(NetworkInitData initData)
        {
            this.name = initData.Name;
            this.magic = initData.Magic;
            this.base58Prefixes = initData.Base58Prefixes;
            this.genesis = initData.GenesisBlock;
            this.hashGenesisBlock = initData.GenesisBlockHash;
            this.nDefaultPort = initData.DefaultPort;
            this.nRPCPort = initData.RPCPort;
            this.nSubsidyHalvingInterval = initData.SubsidyHalvingInterval;
            this.vAlertPubKey = initData.AlertPubKey;
            this.vFixedSeeds = initData.FixedSeeds;
            this.vSeeds = initData.Seeds;
            this._ProofOfLimit = initData.ProofOfLimit;
        }
		private Network(){}
		...
		public static Network Main
		{
			get
			{
				if(_Main == null)
				{
					var instance = new Network(WellKnownNetworkInitDataFactory.InitMain(new NetworkInitData()));
					_Main = instance;
				}
				return _Main;
			}
		...
		public static Network TestNet
		{
			get
			{
				if(_TestNet == null)
				{
					var instance = new Network(WellKnownNetworkInitDataFactory.InitTest(new NetworkInitData()));
					_TestNet = instance;
				}
				return _TestNet;
			}
        ...
        private static List<Network> additionnalNetworks = new List<Network>();
		...
		public static IEnumerable<Network> GetNetworks()
		{
            ...
            foreach (var n in additionnalNetworks) yield return n;
		}
		...
		public static Network GetNetwork(string name)
		{
			...
			switch(name)
			{
				...
				default:
                    var n = GetNetworks().FirstOrDefault(x => x.Name.Equals(name, StringComparison.OrdinalIgnoreCase));
                    if (n==null)
        ...
        public static Network RegisterAdditionnalNetwork(string parentNetwork, Func<NetworkInitData, NetworkInitData> init)
        {
            parentNetwork = parentNetwork.ToLowerInvariant();
            var initData = new NetworkInitData();
            switch (parentNetwork)
            {
                case null:
                case "":
                    initData = new NetworkInitData();
                    break;
                case "main":
                    initData = WellKnownNetworkInitDataFactory.InitMain(initData);
                    break;
                case "testnet":
                case "testnet3":
                    initData = WellKnownNetworkInitDataFactory.InitTest(initData);
                    break;
                case "reg":
                case "regtest":
                    initData = WellKnownNetworkInitDataFactory.InitReg(initData);
                    break;
                default:
                    throw new ArgumentException(String.Format("Invalid parent network name '{0}'", parentNetwork));
            }
            initData = init(initData);
            var network = new Network(initData);
            additionnalNetworks.Add(network);
            return network;
        }
    ...
    public class NetworkInitData
    {
        public string Name;
        public uint Magic;
        public byte[] AlertPubKey;
        public int DefaultPort;
        public int RPCPort;
        public Target ProofOfLimit;
        public int SubsidyHalvingInterval;
        public int SpendableCoinbaseDepth;
        public Block GenesisBlock = new Block();
        public uint256 GenesisBlockHash;
        public List<DNSSeedData> Seeds = new List<DNSSeedData>();
        public List<NetworkAddress> FixedSeeds = new List<NetworkAddress>();
        public byte[][] Base58Prefixes = new byte[12][];

    }

    static class WellKnownNetworkInitDataFactory
    {
        public static NetworkInitData InitReg(NetworkInitData initData)
        {
            initData = InitTest(initData);
            initData.Magic = 0xDAB5BFFA;
            initData.Name = "RegTest";
            initData.SubsidyHalvingInterval = 150;
            initData.ProofOfLimit = new Target(new uint256("7fffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"));
            initData.GenesisBlock.Header.BlockTime = Utils.UnixTimeToDateTime(1296688602);
            initData.GenesisBlock.Header.Bits = 0x207fffff;
            initData.GenesisBlock.Header.Nonce = 2;
            initData.GenesisBlockHash = initData.GenesisBlock.GetHash();
            initData.DefaultPort = 18444;
            //strDataDir = "regtest";
            assert(initData.GenesisBlockHash == uint256.Parse("0x0f9188f13cb7b2c71f2a335e3a4fc328bf5beb436012afca590b1a11466e2206"));

#if !PORTABLE
            initData.Seeds.Clear();  // Regtest mode doesn't have any DNS seeds.
#endif
            return initData;
        }

        public static NetworkInitData InitTest(NetworkInitData initData)
        {
            initData = InitMain(initData);
            initData.Name = "TestNet";
            initData.Magic = 0x0709110B;

            initData.AlertPubKey = DataEncoders.Encoders.Hex.DecodeData("04302390343f91cc401d56d68b123028bf52e5fca1939df127f63c6467cdf9c8e2c14b61104cf817d0b780da337893ecc4aaff1309e536162dabbdb45200ca2b0a");
            initData.DefaultPort = 18333;
            initData.RPCPort = 18332;
            //strDataDir = "testnet3";

            // Modify the testnet genesis block so the timestamp is valid for a later start.
            initData.GenesisBlock.Header.BlockTime = Utils.UnixTimeToDateTime(1296688602);
            initData.GenesisBlock.Header.Nonce = 414098458;
            initData.GenesisBlockHash = initData.GenesisBlock.GetHash();
            assert(initData.GenesisBlockHash == uint256.Parse("0x000000000933ea01ad0ee984209779baaec3ced90fa3f408719526f8d77f4943"));

#if !PORTABLE
            initData.FixedSeeds.Clear();
            initData.Seeds.Clear();
            initData.Seeds.Add(new DNSSeedData("bitcoin.petertodd.org", "testnet-seed.bitcoin.petertodd.org"));
            initData.Seeds.Add(new DNSSeedData("bluematt.me", "testnet-seed.bluematt.me"));
#endif
            initData.Base58Prefixes[(int)Base58Type.PUBKEY_ADDRESS] = new byte[] { (111) };
            initData.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS] = new byte[] { (196) };
            initData.Base58Prefixes[(int)Base58Type.SECRET_KEY] = new byte[] { (239) };
            initData.Base58Prefixes[(int)Base58Type.EXT_PUBLIC_KEY] = new byte[] { (0x04), (0x35), (0x87), (0xCF) };
            initData.Base58Prefixes[(int)Base58Type.EXT_SECRET_KEY] = new byte[] { (0x04), (0x35), (0x83), (0x94) };
            initData.Base58Prefixes[(int)Base58Type.STEALTH_ADDRESS] = new byte[] { 0x2b };
            initData.Base58Prefixes[(int)Base58Type.ASSET_ID] = new byte[] { 115 };
        ...
        public static NetworkInitData InitMain(NetworkInitData initData)
        {
            ...
            initData.GenesisBlock.Transactions.Add(txNew);
            initData.GenesisBlock.Header.HashPrevBlock = 0;
            initData.GenesisBlock.UpdateMerkleRoot();
            initData.GenesisBlock.Header.Version = 1;
            initData.GenesisBlock.Header.BlockTime = Utils.UnixTimeToDateTime(1231006505);
            initData.GenesisBlock.Header.Bits = 0x1d00ffff;
            initData.GenesisBlock.Header.Nonce = 2083236893;

            initData.GenesisBlockHash = initData.GenesisBlock.GetHash();
            assert(initData.GenesisBlockHash == uint256.Parse("0x000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f"));
            assert(initData.GenesisBlock.Header.HashMerkleRoot == uint256.Parse("0x4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b"));
#if !PORTABLE
            initData.Seeds.Add(new DNSSeedData("bitcoin.sipa.be", "seed.bitcoin.sipa.be"));
            initData.Seeds.Add(new DNSSeedData("bluematt.me", "dnsseed.bluematt.me"));
            initData.Seeds.Add(new DNSSeedData("dashjr.org", "dnsseed.bitcoin.dashjr.org"));
            initData.Seeds.Add(new DNSSeedData("bitcoinstats.com", "seed.bitcoinstats.com"));
            initData.Seeds.Add(new DNSSeedData("bitnodes.io", "seed.bitnodes.io"));
            initData.Seeds.Add(new DNSSeedData("xf2.org", "bitseed.xf2.org"));
            initData.Seeds.Add(new DNSSeedData("bitcoin.jonasschnelli.ch", "seed.bitcoin.jonasschnelli.ch"));

#endif
            initData.Base58Prefixes[(int)Base58Type.PUBKEY_ADDRESS] = new byte[] { (0) };
            initData.Base58Prefixes[(int)Base58Type.SCRIPT_ADDRESS] = new byte[] { (5) };
            initData.Base58Prefixes[(int)Base58Type.SECRET_KEY] = new byte[] { (128) };
            initData.Base58Prefixes[(int)Base58Type.ENCRYPTED_SECRET_KEY_NO_EC] = new byte[] { 0x01, 0x42 };
            initData.Base58Prefixes[(int)Base58Type.ENCRYPTED_SECRET_KEY_EC] = new byte[] { 0x01, 0x43 };
            initData.Base58Prefixes[(int)Base58Type.EXT_PUBLIC_KEY] = new byte[] { (0x04), (0x88), (0xB2), (0x1E) };
            initData.Base58Prefixes[(int)Base58Type.EXT_SECRET_KEY] = new byte[] { (0x04), (0x88), (0xAD), (0xE4) };
            initData.Base58Prefixes[(int)Base58Type.PASSPHRASE_CODE] = new byte[] { 0x2C, 0xE9, 0xB3, 0xE1, 0xFF, 0x39, 0xE2 };
            initData.Base58Prefixes[(int)Base58Type.CONFIRMATION_CODE] = new byte[] { 0x64, 0x3B, 0xF6, 0xA8, 0x9A };
            initData.Base58Prefixes[(int)Base58Type.STEALTH_ADDRESS] = new byte[] { 0x2a };
            initData.Base58Prefixes[(int)Base58Type.ASSET_ID] = new byte[] { 23 };
            initData.Base58Prefixes[(int)Base58Type.COLORED_ADDRESS] = new byte[] { 0x13 };
            ...
            for (int i = 0; i < pnSeed.Length; i++)
            {
                ...
                addr.Endpoint = new IPEndPoint(ip, initData.DefaultPort);
                initData.FixedSeeds.Add(addr);
            }
#endif
            return initData;
        }
